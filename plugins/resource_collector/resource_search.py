...
from core.message.message_components import Image, Node, Text, File, Card
...
logger=get_logger()
_zlibrary_client: Optional[Zlibrary] = None
_zlibrary_cache_key: Optional[Tuple[str, str, str]] = None

global operating
operating = {}  #jm文件共享的维护列表


def _build_zlibrary_proxies(config):
...
def _get_zlibrary_client(config) -> Optional[Zlibrary]:
    global _zlibrary_client, _zlibrary_cache_key

    zlibrary_config = config.resource_collector.config["z_library"]
    email = zlibrary_config["email"]
    password = zlibrary_config["password"]
    proxy = config.common_config.basic_config["proxy"]["http_proxy"] or ""

    if not email or not password:
        return None

    cache_key = (email, password, proxy)
    if _zlibrary_client is not None and _zlibrary_cache_key == cache_key:
        return _zlibrary_client

    try:
        _zlibrary_client = Zlibrary(
            email=email,
            password=password,
            proxies=_build_zlibrary_proxies(config),
        )
        _zlibrary_cache_key = cache_key
        logger.info("✅ z_library 登陆成功")
    except Exception as e:
        _zlibrary_client = None
        _zlibrary_cache_key = cache_key
...
async def search_book_info(bot,event,config,info):
    ...
    if user_info.permission >= config.resource_collector.config["z_library"]["search_operate_level"]:
        zlibrary_client = _get_zlibrary_client(config)
        if zlibrary_client is None:
            await bot.send(event, "z_library 未配置完成或登录失败")
            return

        await bot.send(event, "正在搜索中，请稍候...")
        result =await search_book(zlibrary_client, info, config.resource_collector.config["z_library"]["search_num"])
        forward_list = []
        for r in result:
    ...
    if user_info.permission >= config.resource_collector.config["z_library"]["download_operate_level"]:
        ...
        try:
            with ThreadPoolExecutor() as executor:
                path = await loop.run_in_executor(executor,download_book,zlibrary_client,book_id.strip(),hash.strip())
