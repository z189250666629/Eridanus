...
dynamic_imports = {
    "plugins.resource_collector.resource_search": [
        "search_book_info", "call_asmr", "call_download_book","call_jm"
    ],
    "plugins.resource_collector.engine_search": ["search_net", "read_html"],
    "plugins.resource_collector.func_collection": ["iwara_search", "iwara_tendency"],
}
...
def register_services(registry, provider: str = "resource_collector", **_kwargs):
    ...
    service_map = {
        "random_asmr_100": random_asmr_100,
        "resource_collector.random_asmr_100": random_asmr_100,
        "parse_from_asmr_id": parse_from_asmr_id,
        "resource_collector.parse_from_asmr_id": parse_from_asmr_id,
    }
    ...
    for service_name, service in service_map.items():
        registry.register(
            ...
            service,
            provider=provider,
            metadata={"plugin": "resource_collector"},
        )

    return list(service_map.keys())


def unregister_services(registry, provider: str = "resource_collector", **_kwargs):
