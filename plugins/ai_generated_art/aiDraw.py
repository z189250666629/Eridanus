...
from plugins.ai_generated_art.service.modelscope_text2img import modelscope_drawer
from plugins.ai_generated_art.service.hf_t2i import hf_drawer
from plugins.ai_generated_art.service.setu_moderate import pic_audit_standalone
from core.database.user import get_user, User
from core.toolkit.random_utils import random_str
from plugins.ai_generated_art.service.aiDraw import n4, n3, SdDraw0, getloras, getcheckpoints, ckpt2, n4re0, n3re0, \
    SdmaskDraw, getsampler, getscheduler, interrupt, skipsd, SdOutpaint, get_img_info, aiArtModerate
from plugins.ai_generated_art.service.wildcard import get_available_wildcards, replace_wildcards
from core.toolkit.compat_utils import download_img, url_to_base64, parse_arguments, get_img, delay_recall
...
def _get_bing_dalle3():
    registry = get_service_registry()
    service = registry.get("bing_dalle3") or registry.get("basic_plugin.bing_dalle3")
    if service is not None:
        return service
    return import_module("plugins.basic_plugin.service.ai_text2img").bing_dalle3


def _get_flux_ultra():
    registry = get_service_registry()
    service = registry.get("flux_ultra") or registry.get("basic_plugin.flux_ultra")
    if service is not None:
        return service
    return import_module("plugins.basic_plugin.service.ai_text2img").flux_ultra


def _get_doubao():
    registry = get_service_registry()
    service = registry.get("doubao") or registry.get("basic_plugin.doubao")
    if service is not None:
        return service
    return import_module("plugins.basic_plugin.service.ai_text2img").doubao


def _get_anime_trace():
    registry = get_service_registry()
...
async def call_text2img2(bot, event, config, tag):
    ...
    if user_info.permission >= config.ai_generated_art.config["ai绘画"]["内置ai绘画1所需权限等级"] and \
        ...
        bing_dalle3 = _get_bing_dalle3()
        flux_ultra = _get_flux_ultra()
        doubao = _get_doubao()
