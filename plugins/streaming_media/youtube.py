...
def _get_parse_from_asmr_id():
    registry = get_service_registry()
    service = (
        registry.get("parse_from_asmr_id")
        or registry.get("resource_collector.parse_from_asmr_id")
    )
    ...
    async def _download_video():
        if platform == "youtube":
            ...
            if match:
                ...
                proxy = config.common_config.basic_config["proxy"]["http_proxy"]
                parse_from_asmr_id = _get_parse_from_asmr_id()
