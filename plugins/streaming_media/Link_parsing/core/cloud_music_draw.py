...
def draw_netease_music_card(data, filepath, song_id):
    ...
    font_bold_path = DRAW_FONT_DIR / "LXGWWenKai-Bold.ttf"
    print(f"字体文件路径: {font_path}")
    try:
        font = ImageFont.truetype(str(font_path), font_size)
        font_small = ImageFont.truetype(str(font_path), 40)
        font_bold = ImageFont.truetype(str(font_bold_path), font_size)  # 加粗字体
        font_small_bold = ImageFont.truetype(str(font_bold_path), 40) # 加粗小字体
        font_creator = ImageFont.truetype(str(font_path), 30)
