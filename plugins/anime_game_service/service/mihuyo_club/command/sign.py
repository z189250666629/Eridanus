...
MIHUYO_BACKGROUND = Path(__file__).resolve().parent / "米游社.png"


def _mihuyo_avatar_card(user_id: int, uid: str) -> dict:
    card = {
        'type': 'avatar',
        'subtype': 'common',
    ...
    if MIHUYO_BACKGROUND.exists():
        card['background'] = str(MIHUYO_BACKGROUND)
    return card
            ...
            if (get_info_status and not info.is_sign) or not get_info_status:
                ...
                if not sign_status and user.enable_notice:
                    ...
                    return_json['manshuo_draw'] = [
                       _mihuyo_avatar_card(user_id, UID),
