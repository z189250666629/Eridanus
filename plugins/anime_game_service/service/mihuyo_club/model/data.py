...
class BBSCookies(BaseModelWithSetter, BaseModelWithUpdate):
    ...
    def dict(self, *,
        ...
        cookies_dict = super().dict(include=include, exclude=exclude, by_alias=by_alias,
...
class UserAccount(BaseModelWithSetter):
    ...
    """手机号"""
    cookies: BBSCookies = None
    """Cookies"""
    address: Optional[Address] = None
    """收货地址"""

    device_id_ios: str = None
    """iOS设备用 deviceID"""
    device_id_android: str = None
    """安卓设备用 deviceID"""
...
class UserData(BaseModelWithSetter):
    ...
    """储存一些已绑定的账号数据"""
    target_sign_game: Optional[str] = None
    """签到时候的默认游戏"""
...
class PluginDataManager:
    ...
    def load_plugin_data(cls):
        ...
        if plugin_data_path.exists() and plugin_data_path.is_file():
            try:
                ...
                logger.error(f"读取插件数据文件失败，请检查插件数据文件 {plugin_data_path} 格式是否正确")
                raise
            except Exception:
                logger.error(
            ...
            try:
                ...
                logger.error(f"创建插件数据文件失败，请检查是否有权限读取和写入 {plugin_data_path}")
                raise
            else:
    ...
    def write_plugin_data(cls):
        ...
        try:
            ...
            logger.error("数据对象序列化失败，可能是数据类型错误")
            return False
        else:
            with open(plugin_data_path, "w", encoding="utf-8") as f:
                f.write(str_data)
            #cls.load_plugin_data()
            return True


PluginDataManager_Init = PluginDataManager.load_plugin_data()
