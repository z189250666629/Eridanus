...
class PluginConfig(BaseSettings):
    ...
    preference: ClassVar[Preference] = Preference(geetest_url=None)
    #good_list_image_config = GoodListImageConfig()
    good_list_image_config: ClassVar[GoodListImageConfig] = GoodListImageConfig()
...
if plugin_config_path.exists() and plugin_config_path.is_file():
    ...
    try:
        ...
        logger.error(f"创建插件配置文件失败，请检查是否有权限读取和写入 {plugin_config_path}")
        raise
    else:
